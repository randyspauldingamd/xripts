...
if [ $# -eq 0 ]; then
	echo "Usage: gR-docker  <docker_image|last>  [once]"
	echo "- to see a list of images, run: 'docker image list'"
	if [ -n "$test_tag" ]; then
		echo "- run 'gR-docker last' to run container  '$test_tag'  again"
	fi
...
if ! ($SU docker run -w $wd/repos -v $wd:$wd -it --name $DKR_NAME --hostname $DKR_NAME --privileged --device=/dev/kfd --device=/dev/dri $miopen_tag); then
	$SU docker run -w $wd -v $wd:$wd -it --name $DKR_NAME --hostname $DKR_NAME --privileged --device=/dev/kfd --device=/dev/dri $miopen_tag
fi
